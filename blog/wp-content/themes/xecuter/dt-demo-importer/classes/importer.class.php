...
class DT_Demo_Importer extends DT_Demo_Importer_Abstract {
 ...
 public function admin_page() {
  ...
  <div class="wrap dt-importer">
    <h2><?php _e( 'وارد کردن طرح های آمده', 'dt-importer' ); ?></h2>
      ...
      <?php
        foreach ($this->items as $item => $value ) :
          ...
          if (!empty($opt[$item])) {
            ...
            $btn_text .= __( 'دوباره وارد کنید', 'dt-importer' );
            $status .= __( 'وارد شده', 'dt-importer' );
          } else {
            $btn_text .= __( 'وارد کردن', 'dt-importer' );
            $status .= __( 'وارد نشده', 'dt-importer' );
          }
      ?>
        <div class="dt-demo-item <?php echo esc_attr($imported_class); ?>" data-dt-importer>
          ...
          <div class="dt-tag">
            ...
            <?php
               $image_url = '';
              if (file_exists(DT_IMPORTER_CONTENT_DIR . $item . '/'.$item.'.jpg')) {
                $image_url = DT_IMPORTER_CONTENT_URI . $item . '/'.$item.'.jpg';
              } else {
                $image_url = DT_IMPORTER_URI . '/assets/img/'.$item.'.jpg';
              }
            ?>
...
deactivate_plugins( '/wordpress-importer/wordpress-importer.php' );
	 wp_delete_nav_menu('menu-1');
	 wp_delete_nav_menu('menu-2');
 	 
	 wp_create_nav_menu('menu-1');
	 wp_create_nav_menu('menu-2');
  
  
   
    $id = $_POST['id'];
		$item = get_page_by_path( $id );					
 	 wp_delete_post($item->ID,true); 
     // Import XML Data
    $this->import_xml_data();
     // Setup Option Data from Codestar
     $this->import_cs_options_data();
    $this->import_cs_widget_data();
  ...
  public function import_xml_data() {
    ...
    $file = DT_IMPORTER_CONTENT_DIR  . '/'.$id.'/'.$id.'.xml';
  ...
  public function import_cs_options_data() {
    $id = $_POST['id'];
    $file = DT_IMPORTER_CONTENT_DIR . $id . '/'.$id.'.txt';
    ...
    if ( file_exists( $file ) ) {
      ...
      $data = file_get_contents( $file );
		$ada_data= unserialize(urldecode($data));
			$xecuter_page_builder = get_page_by_path( $id );								

 		$ada_data['xecuter_page_builder'] =$xecuter_page_builder->ID;
 
		update_option( 'theme_mods_xecuter', $ada_data );

    }
  }

  public function import_cs_widget_data() {
	  
    $id = $_POST['id'];

    $file = DT_IMPORTER_CONTENT_DIR  . '/widgets.wie';
		xecuter_process_import_file($file );
