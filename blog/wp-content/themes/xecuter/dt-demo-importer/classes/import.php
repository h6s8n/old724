 ...
 * @since 0.3
 */
function xecuter_upload_import_file() {

	// Check nonce for security since form was posted.
	// check_admin_referer prints fail page and dies.
	if ( ! empty( $_POST ) && ! empty( $_FILES['xecuter_import_file'] ) && check_admin_referer( 'xecuter_import', 'xecuter_import_nonce' ) ) {
		...
		add_filter( 'wp_check_filetype_and_ext', 'xecuter_disable_real_mime_check', 10, 4 );

		// Uploaded file.
		$uploaded_file = $_FILES['xecuter_import_file'];
		...
		// Process import file.
		xecuter_process_import_file( $file_data['file'] );

	}

}

add_action( 'load-tools_page_widget-importer-exporter', 'xecuter_upload_import_file' );
 ...
 * @global string $xecuter_import_results
 */
function xecuter_process_import_file( $file ) {

	global $xecuter_import_results;
	...
	$xecuter_import_results = xecuter_import_data( $data );
 ...
 */
function xecuter_import_data( $data ) {
	...
	// Hook before import.
	do_action( 'xecuter_before_import' );
	$data = apply_filters( 'xecuter_import_data', $data );

	// Get all available widgets site supports.
	$available_widgets = xecuter_available_widgets();
	...
	foreach ( $data as $sidebar_id => $widgets ) {
		...
		foreach ( $widgets as $widget_instance_id => $widget ) {
			...
			// Ideally the newer xecuter_widget_settings_array below will be used instead of this.
			$widget = apply_filters( 'xecuter_widget_settings', $widget );
			...
			// This is preferred over the older xecuter_widget_settings filter above
			// Do before identical check because changes may make it identical to end result (such as URL replacements).
			$widget = apply_filters( 'xecuter_widget_settings_array', $widget );
			...
			if ( ! $fail ) {
				...
				do_action( 'xecuter_after_widget_import', $after_widget_import );
	...
	// Hook after import.
	do_action( 'xecuter_after_import' );

	// Return results.
	return apply_filters( 'xecuter_import_results', $results );
