...
function optionsframework_add_admin() {
	...
	$of_page = add_theme_page( THEMENAME, 'پنل قالب', 'edit_theme_options', 'optionsframework', 'optionsframework_options_page');
...
function of_load_only() 
{
	...
	//xecuter Eite
	wp_enqueue_script('jquery-input-mask', get_template_directory_uri() .'/framework/assets/js/jquery.maskedinput-1.2.2.js', array( 'jquery' ));
	wp_enqueue_script('tipsy',  get_template_directory_uri() .'/framework/assets/js/jquery.tipsy.js', array( 'jquery' ));
	//wp_enqueue_script('color-picker', ADMIN_DIR .'assets/js/colorpicker.js', array('jquery'));
	wp_enqueue_script('cookie',  get_template_directory_uri() .'/framework/assets/js/cookie.js', 'jquery');
	wp_enqueue_script('smof', get_template_directory_uri() .'/framework/assets/js/smof.js', array( 'jquery' ));
 	wp_enqueue_script('predefined', get_template_directory_uri() .'/framework/assets/js/predefined.js', array( 'jquery' ));
...
function of_ajax_callback() 
{
	...
	if($save_type == 'upload')
	{
		...
		$uploaded_file = wp_handle_upload($filename,$override);
		 ...
		 if(!empty($uploaded_file['error'])) {echo wp_kses_post('Upload Error: ' . $uploaded_file['error']); }	
		 else { echo wp_kses_post($uploaded_file['url']); } // Is the Response
	...
	elseif($save_type == 'import_options'){
		...
		$smof_data = unserialize(urldecode($_POST['data'])); //100% safe - ignore theme check nag
		of_save_options($smof_data);
